...
def get_sets(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    sets = routes.get_sets(db = db, skip = skip, limit = limit)
    return sets
...
def update_set(item: schemas.SetCreate, item_id: int, db: Session = Depends(get_db)):
    set = routes.update_set(item_id= item_id, db = db, item = item)
    if not set:
        raise HTTPException(status_code= 404, detail= "Flashcard not found.")
    return set

@app.delete("/delete_set/{id}")
def delete_set(item_id: int, db: Session = Depends(get_db)):
    set = routes.delete_set(item_id= item_id, db = db)
    if not set:
        raise HTTPException(status_code= 404, detail= "Flashcard not found.")
    return {"message": "Flashcard deleted successfully."}
